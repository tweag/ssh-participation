...
let
  ...
  login = pkgs.writeShellScript "login" ''
    ...
    user=''${SSH_USER,,}
    # FIXME: Lock user to prevent race conditions, also in clear script
  ...
  clear = pkgs.writeShellScriptBin "ssh-participation-clear" ''
    ...
    runDir=/var/lib/bouncer/runs/$(date -Iseconds)

    cd /home/participants

    for user in *; do
      mkdir -p "$runDir"
      cp -r "$user" "$runDir"/"$user"
      uid=$(id -u "$user")
      loginctl terminate-user "$user" 2>/dev/null || true
      userdel --remove --force "$user" 2>/dev/null
      rm /var/lib/bouncer/slices/user-"$uid".slice.d
    done
  '';
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.ssh-participation = {
      ...
      script = ''
        ...
        SSH_PASSWORD_FILE="$tmp" exec ${ssh-participation}/bin/ssh-participation sudo -E ${login}
      '';
